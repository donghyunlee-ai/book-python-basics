# ch10-06 수학 도우미를 다듬다 — 컴프리헨션·zip 실전 (본문 게재 코드 원본 — writer 기계적 추출, 미실행)
# 검증 대상(fact-checker 실행 예정): python3 (파일 산출물 primes.txt 정리 포함)
...
# 검증용 안전판: 본문 코드가 만든 산출물 파일을 정리합니다(원본에는 없는 정리 단계).
