# 4장 7절 [AI와 짝 코딩] 딕셔너리 코드 요청 — 없는 키를 의심하라 (AI가 내놓기 쉬운 오답 코드)
# 오답 코드 1개와, 해설 내용을 하나씩 확인하는 코드

# --- 오답 코드 (AI가 돌려주기 쉬운 코드 — 예시 데이터에선 성공) ---
scores = {"김파이": 90, "이코드": 85}
print(scores["김파이"])  # 출력: 90
assert scores["김파이"] == 90

# --- 해설: 예시 밖 이름을 넣는 순간 죽는다 (에러 마지막 줄만 확인) ---
try:
    scores["김코드"]
    ...
    print(last_line)
    assert last_line == "KeyError: '김코드'"  # 책에 옮긴 에러 문구와 같다
