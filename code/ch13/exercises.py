# 13장 7절 연습문제
# 연습문제 4개(예측형 1 + 구현형 1 + 리뷰형 2)를 실행해 볼 수 있게 묶었다.
# input()으로 입력을 기다리는 예제는 unittest.mock.patch로 입력값을 미리 정해 두었다.
...
def q3_buggy_demo(max_attempts=3):
    # 책의 문제 코드는 while True: ... except: 로 무한 반복이지만(오타가 고쳐지지
    # 않으므로 매번 같은 거짓 안내가 나온다), 이 파일이 멈추지 않고 끝까지 실행되도록
    # 시도 횟수 상한을 둔다.
    attempts = 0
