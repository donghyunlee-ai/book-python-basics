# 13장 4절 뒷정리와 정직한 거절 — finally, else, raise
# 이 절의 예제들을 묶었다.
import os
...
def scenario_3_uncaught_demo():
    # 책에 실린 형태(주석을 풀고 실행하면 TypeError가 발생하며 프로그램이 멈춥니다):
    # divide_and_cleanup(10, "2")
    # 이 바깥 부분은 finally가 에러 직전에 실제로 찍히는지, 프로그램을
    # 통째로 멈추지 않고 이 파일에서 확인하려고 둔 틀입니다(책에는 안쪽 코드만
    # 나옵니다).
    try:
        divide_and_cleanup(10, "2")
...
# --- with의 정체 다시 보기: try/finally와 with는 같은 일을 한다 ---
...
# --- raise: 이번엔 우리가 던진다 ---
class BankAccount:
    # 11장에서 완성한 BankAccount(인자 3개)를 그대로 재현한 최소 틀입니다
    # (책에는 deposit 메서드만 나옵니다).
